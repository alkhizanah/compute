...
static inline ExprIdx pool_push_var(Pool *pool, const char *variable) {
    return pool_push_expr(pool, EXPR_VAR,
                          (ExprPayload){.var= variable});
}

static inline ExprIdx pool_push_val(Pool *pool, double val) {
    return pool_push_expr(pool, EXPR_VAL,
                          (ExprPayload){.val = val});
}
...
static inline ExprIdx pool_push_fn(Pool *pool, const char *name,
                               const char **parameters, uint32_t parameters_len,
                               ExprIdx body) {
    assert(parameters_len != 0 &&
           "mathematical functions can not have 0 parameters");

    ExprIdx first_parameter = pool_push_var(pool, parameters[0]);

    for (size_t i = 1; i < parameters_len; i++) {
        pool_push_var(pool, parameters[i]);
    }

    return pool_push_expr(
        pool, EXPR_FN,
        (ExprPayload){.fn = {.name = name,
